...
public class Football03 {
    public static void main(String[] args) {
        ...
        OptionalInt minSequential = table.stream().mapToInt(Club::getPoints).min();
        if (minSequential.isPresent()) {
            System.out.printf("Lowest number of points is %d\n", minSequential.getAsInt());
        } else {
        ...
        OptionalInt minParallel = table.parallelStream().mapToInt(Club::getPoints).min();
        if (minParallel.isPresent()) {
            System.out.printf("Lowest number of points is %d\n", minParallel.getAsInt());
        } else {
            System.out.println("min failed");
        }
