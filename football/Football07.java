...
public class Football07 {
    public static void main(String[] args) {
        ...
        List<Club> bestGoalDifferenceClubsSequential = clubList.stream()
                .sorted((c1, c2) -> Integer.compare(c2.getGoalDifference(), c1.getGoalDifference()))
                .limit(3)
                .collect(Collectors.toList());

        System.out.println("Clubs with the Best Goal Differences (Sequential):");
        bestGoalDifferenceClubsSequential.forEach(System.out::println);
        long endTimeSequential = System.currentTimeMillis();
        System.out.println("Sequential Execution Time: " + (endTimeSequential - startTimeSequential) + " ms");

        System.out.println("\nParallel Stream:");
        long startTimeParallel = System.currentTimeMillis();
        List<Club> bestGoalDifferenceClubsParallel = clubList.parallelStream()
                .sorted((c1, c2) -> Integer.compare(c2.getGoalDifference(), c1.getGoalDifference()))
                .limit(3)
                .collect(Collectors.toList());

        System.out.println("Clubs with the Best Goal Differences (Parallel):");
        bestGoalDifferenceClubsParallel.forEach(System.out::println);
