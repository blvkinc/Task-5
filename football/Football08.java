...
public class Football08 {
    public static void main(String[] args) {
        ...
        List<Club> highRedCardClubsSequential = clubList.stream()
                .sorted((c1, c2) -> Integer.compare(c2.getRedCards(), c1.getRedCards()))
                .limit(3)
                .collect(Collectors.toList());

        System.out.println("Clubs with the Highest Number of Red Cards (Sequential):");
        highRedCardClubsSequential.forEach(System.out::println);
        long endTimeSequential = System.currentTimeMillis();
        System.out.println("Sequential Execution Time: " + (endTimeSequential - startTimeSequential) + " ms");

        System.out.println("\nParallel Stream:");
        long startTimeParallel = System.currentTimeMillis();
        List<Club> highRedCardClubsParallel = clubList.parallelStream()
                .sorted((c1, c2) -> Integer.compare(c2.getRedCards(), c1.getRedCards()))
                .limit(3)
                .collect(Collectors.toList());

        System.out.println("Clubs with the Highest Number of Red Cards (Parallel):");
        highRedCardClubsParallel.forEach(System.out::println);
